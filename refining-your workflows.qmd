...
```{r}
#| label: tbl-island-penguin
#| tbl-cap: "Counts of Palmer penguin in each island."

island_counts <- table(penguins$island)
kable(island_counts, col.names = c("Island", "Count"))
```
@tbl-island-penguin shows the count of penguins in each
...
knitr::kable(x = flipper_len_lm_coeff,
             col.names = c("MLE", "Std. Err"),
             )
flipper_len_lm
```
## Basic Plots {#island-plots}

@island-plots focuses on analysis on the penguins of each island
...
the cdf associated with @eq-cauchy is given by
